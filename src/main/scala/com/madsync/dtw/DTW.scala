...
import com.madsync.dtw.domain.{ CostMatrix, IndexConstraints, MatrixEntry, PassthroughIndexConstraints, Space, TimeSeriesElement }
...
class DTW[T](space: Space[T]) {
  ...
  /**
   * Given a time series with indices m = 1..M and a time series with indices n = 1..N,
   * a cost matrix at index (i,j) is a representation of the distance contribution from
   * time series element m=i, n=j
   *
   * @param left        the left time series operand (indices m)
   * @param right       the right time series operand (indices n)
   * @param constraints   applied constraints that bound the windows developed
   * @return
   */
